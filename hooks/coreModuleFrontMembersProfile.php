...
class membermap_hook_coreModuleFrontMembersProfile extends _HOOK_CLASS_
{
	...
	protected function _saveMember( $form, array $values ): void
	{
		...
		if ( \is_array( $values ) AND \count( $values ) AND \IPS\Settings::i()->membermap_monitorLocationField AND 
			! $this->member->members_bitoptions['bw_is_spammer'] AND \count( explode( ',', \IPS\Settings::i()->membermap_profileLocationField ) ) )
		{
			...
			{
				/* borisMemberLocation */
				if ( $fieldKey == 999999 AND $hasMemberLocationPlugin )
				{
